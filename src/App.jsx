              ...
              <div className="flex-1 relative">
                ...
                {/* Autocomplete Dropdown */}
                    {showSuggestions && addressSuggestions.length > 0 && (
                      <div className="absolute top-full left-0 right-0 mt-2 bg-emerald-900/98 backdrop-blur-md border border-white/20 rounded-xl shadow-2xl z-50 overflow-hidden max-h-96 overflow-y-auto p-2">
                        <div className="px-4 py-2 mb-2">
                          <p className="text-xs text-emerald-200 font-light">Tap to select, or type your complete address</p>
                        </div>
                        {addressSuggestions.map((suggestion, index) => (
                          <button
                            key={index}
                            onClick={() => selectAddress(suggestion)}
                            className="w-full text-left px-4 py-4 mb-2 bg-emerald-950/50 hover:bg-lime-400/20 transition-colors rounded-lg text-white font-light text-sm border border-white/10"
                          >
                            <div className="flex items-center gap-3">
                              <Home className="w-5 h-5 text-lime-400 flex-shrink-0" />
