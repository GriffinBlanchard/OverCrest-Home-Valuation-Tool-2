              ...
              <div className="flex-1 relative">
                ...
                {/* Autocomplete Dropdown */}
                    {showSuggestions && addressSuggestions.length > 0 && (
                      <div className="absolute top-full left-0 right-0 mt-3 space-y-3 z-50 max-h-96 overflow-y-auto px-1">
                        {addressSuggestions.map((suggestion, index) => (
                          <button
                            key={index}
                            onClick={() => selectAddress(suggestion)}
                            className="w-full text-left p-5 bg-white text-emerald-900 hover:bg-lime-400 transition-all rounded-2xl shadow-xl border-2 border-white/30 backdrop-blur-sm"
                          >
                            <div className="flex items-center gap-4">
                              <div className="w-12 h-12 rounded-full bg-lime-400 flex items-center justify-center flex-shrink-0">
                                <Home className="w-6 h-6 text-emerald-900" />
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className="font-medium text-base break-words">{suggestion}</p>
                              </div>
                            </div>
                          </button>
                        ))}
